...
async function getDNSDelay() {
  try {
    const { delay } = await httpMethod.get('/v1/test/dns_delay');
    return `[Latency] ${(delay * 1000).toFixed(0)} ms`;
  } catch (error) {
...
async function getNetworkInfo(retryTimes = 5, retryInterval = 1000) {
  ...
  while (retryTimes > 0) {
    try {
      const [ipApiResponse, dnsApiResponse, stunIP, dnsDelay] = await Promise.all([
